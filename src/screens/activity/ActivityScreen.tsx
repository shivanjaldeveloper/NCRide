...
const UPCOMING: ActivityItem[] = [
  { id: 'NR-RV-892412', icon: 'reserve', title: 'Reserve · Sector 62 → Airport T3', sub: 'NCRide Prime · Sat 22 Mar · 05:00', tag: 'Scheduled', tagColor: Colors.blue, tagBg: '#E8F1FF' },
  { id: 'NR-IC-447721', icon: 'intercity', title: 'Intercity · Noida → Gurugram', sub: 'NCRide XL · Fri 21 Mar · 09:00', tag: 'Confirmed', tagColor: Colors.green, tagBg: '#E9F8E4' },
];

const PAST: ActivityItem[] = [
  { id: 'NR-N22-78421', icon: 'taxi', title: 'Sector 62, Noida → Connaught Place', sub: 'Auto Rickshaw · Yesterday', tag: '₹110 · Paid', tagColor: Colors.ink, tagBg: Colors.bgOffWhite },
  { id: 'NR-CR-552411', icon: 'courier', title: 'Documents · Sec 62 → Connaught Place', sub: 'Same-city courier · 2 days ago', tag: '₹99 · Paid', tagColor: Colors.ink, tagBg: Colors.bgOffWhite },
  { id: 'NR-N22-78101', icon: 'car', title: 'Sector 18 → Botanical Garden Metro', sub: 'E-Rickshaw · 2 days ago', tag: '₹45 · Paid', tagColor: Colors.ink, tagBg: Colors.bgOffWhite },
  { id: 'NR-N22-77501', icon: 'taxi', title: 'Noida Sector 16 → India Gate', sub: 'Auto Rickshaw · 5 days ago', tag: '₹96 · Paid', tagColor: Colors.ink, tagBg: Colors.bgOffWhite },
  { id: 'NR-N22-77302', icon: 'taxi', title: 'Indirapuram → Hauz Khas', sub: 'Auto · 7 days ago', tag: '₹128 · Paid', tagColor: Colors.ink, tagBg: Colors.bgOffWhite },
];
    ...
    <View style={styles.root}>
      ...
      <View style={styles.segWrap}>
        <View style={styles.seg}>
          {([['upcoming', t.activity.upcoming], ['past', t.activity.past]] as const).map(([id, label]) => (
            <TouchableOpacity key={id} activeOpacity={0.8} onPress={() => setTab(id)} style={[styles.segBtn, tab === id && styles.segBtnActive]}>
      ...
      <ScrollView contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}>
        {list.map(item => (
          <TouchableOpacity key={item.id} activeOpacity={0.8} onPress={() => navigation.navigate('BookingDetail', { id: item.id, title: item.title, icon: item.icon })}>
            <NCCard pad={fscale(14)} style={styles.card}>
              <View style={styles.cardRow}>
                <View style={styles.itemIcon}><Icon name={item.icon} size={20} stroke={Colors.ink} /></View>
                ...
                <View style={styles.itemRight}>
                  <View style={[styles.tagChip, { backgroundColor: item.tagBg }]}>
...
const styles = StyleSheet.create({
  ...
  listContent: { paddingHorizontal: Spacing.screen, paddingBottom: fscale(110), gap: Spacing.sm },
  card: {},
